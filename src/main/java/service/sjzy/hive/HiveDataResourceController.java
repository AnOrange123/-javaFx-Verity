...
/**
 * ClassName:HiveDataResourceController
 ...
 */
public class HiveDataResourceController {
	@FXML
	...
	public void createHiveDataResourceWindow(ActionEvent event) throws IOException, SQLException {
		//获取数据资源界面窗口
		FXMLLoader fxmlLoader = new FXMLLoader(getClass().getClassLoader().getResource("sjzy/hive/HiveDataResource.fxml"));
		VBox hiveDataResourceContent = fxmlLoader.load();

		//设置数据库菜单
		setDatabaseMenu(hiveDataResourceContent);

		EmptyMainController.replaceMainModule(event,hiveDataResourceContent);
	}

	//设置数据库菜单
	public static void setDatabaseMenu(VBox hiveDataResourceContent) throws SQLException, IOException {
		ScrollPane scrollPane = (ScrollPane) hiveDataResourceContent.lookup("#scroll");
		VBox databaseMenu = (VBox) scrollPane.getContent();

		String querySql = String.format(SqlConstant.database_show);
		List<LinkedHashMap<Object, Object>> queryResult = HiveUtil.processHql(querySql, null);

		//遍历数据库名查询结果，逐行添加到文件树
		databaseMenu.getChildren().clear();
		ToggleGroup group = new ToggleGroup();
		for (LinkedHashMap<Object, Object> row : queryResult) {
			HBox hBox = addRow(row, hiveDataResourceContent, group);
			databaseMenu.getChildren().add(hBox);
		}
	}

	//添加一行信息
	private static HBox addRow(LinkedHashMap<Object, Object> row, VBox hiveDataResourceContent, ToggleGroup group) {
		HBox hBox = new HBox();
