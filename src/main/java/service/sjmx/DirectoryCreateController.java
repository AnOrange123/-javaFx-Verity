...
public class DirectoryCreateController {
	...
	void confirmButtonClick(ActionEvent event) throws SQLException, IOException, URISyntaxException, InterruptedException {
		...
		try {
			...
			//刷新文件树
			if (selectItem != null){
				SJMXDetailController.makeBranch(fileName.getText(), selectItem, SomeConstant.DIRECTORY);
			}else {
				SJMXDetailController.makeBranch(fileName.getText(), treeView.getRoot(), SomeConstant.DIRECTORY);
			}
	...
	public void createConfirmWindow(String projectName, String path, TreeItem<String> selectedItem, TreeView<String> treeView) throws IOException {
		DirectoryCreateController.projectName = projectName;
		selectItem = selectedItem;
		DirectoryCreateController.path = path;
		DirectoryCreateController.treeView = treeView;

		FXMLLoader fxmlLoader = new FXMLLoader(SJKFController.class.getClassLoader().getResource("sjmx/DirectoryCreate.fxml"));
